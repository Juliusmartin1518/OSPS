...
function checkTimeQuantumInput() {
    ...
    // Show or hide the Time Quantum field based on the selected algorithm
    if (selectedAlgorithm === "rr") {
        timeQuantumElement.remove("hide"); // Show if Round Robin is selected
    } else {
        timeQuantumElement.add("hide"); // Hide for other algorithms
    }
}

// Add an event listener to the dropdown to trigger checkTimeQuantumInput when the selection changes
...
function showGanttChart(output, outputDiv) {
    ...
    output.schedule.forEach((element) => {
        if (element[0] === -1) {
            // If CPU is idle, add an interval with "I" and the number of seconds
            ganttChartData.push([
                "Interval",
                "I - " + element[1] + "s",
                startGantt * 1000,
    ...
    // Sort and draw the chart
    ganttChartData.sort((a, b) => parseInt(a[1].substring(1)) - parseInt(b[1].substring(1)));
    ...
    function drawGanttChart() {
        ...
        var options = {
            timeline: { showRowLabels: false, avoidOverlappingGridLines: true }
        };
...
document.querySelector(".calculate").onclick = () => {
    ...
    if (selectedAlgorithm === 'rr') {
        // Execute Round Robin specific function
        roundrobin(mainInput, mainUtility, mainOutput);
    } else {
        // Execute other algorithm functions
